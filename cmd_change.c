...

{
  ...
  while ((c = *s) != 0) {				/* repeat while there's stuff to do */
    ...
    if (c == ')') return (orig - (++ s));		/* if terminating on a ')', return negative char count */
...

{
  ...
  if (rc < 0) {
    *s_r -= rc;						/* increment past the terminating ')' */
    rc = 1;						/* and set up successful return status */
  }
...

{
  ...
  while (*linebf != 0) {				/* process command string characters */
    ...
    if ((linebf[0] == 'I') || (linebf[0] == 'i')) {	/* I - supplies characters for the search */
